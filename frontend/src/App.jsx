...
import SuivreCours     from './pages/suivreCours'
...
function App() {
  ...
  const [idCoursActif, setIdCoursActif] = useState(null)
  ...
  const handleProfilMisAJour = (u) => setUser(u)
  ...
  return (
    <div>
      ...
      {page === 'accueil'         && <Accueil user={user} onDeconnexion={handleDeconnexion}
          ...
          allerEspaceAdmin={() => nav('espace-admin')}
          allerSuivreCours={(idCours) => { setIdCoursActif(idCours); nav('suivre-cours') }}
          allerModifierProfil={() => nav('modifier-profil')}
        />}
      ...
      {page === 'espace-admin'    && <EspaceAdmin user={user} onRetour={() => nav('accueil')} />}
      {page === 'suivre-cours'    && <SuivreCours user={user} idCours={idCoursActif} onRetour={() => nav('accueil')} />}
      {page === 'modifier-profil' && <ModifierProfil user={user} onRetour={() => nav('accueil')} onProfilMisAJour={handleProfilMisAJour} />}
    </div>
  )
}
