...
function Quiz({ user, onRetour }) {
  ...
  const [dataQuiz, setDataQuiz] = useState({ titre: '', description: '', dureeLimit: '', notePassage: 60, nbTentatives: 3, estQuizFinal: false })
  ...
  const detectChangeQuiz = (e) => {
    const { name, value, type, checked } = e.target
    setDataQuiz(a => ({ ...a, [name]: type === 'checkbox' ? checked : value }))
  }
  ...
  const handleSubmitQuiz = (e) => {
    ...
    api.post(`/quiz?idCours=${idCoursChoisi}`, {
      ...
      estQuizFinal: dataQuiz.estQuizFinal
    })
      .then(res => {
        setLoading(false)
        setMessage({ type: 'success', texte: 'Quiz cree !' })
        setDataQuiz({ titre: '', description: '', dureeLimit: '', notePassage: 60, nbTentatives: 3, estQuizFinal: false })
                  ...
                  <div key={q.idQuiz} className="quiz-card">
                    <div className="quiz-card-header">
                      ...
                      <div>
                        ...
                        <p>{q.description}</p>
                        {q.estQuizFinal && (
                          <span className="badge-quiz-final">
                            <i className='bx bx-certification'></i>Quiz final
                          </span>
                        )}
                      </div>
                    </div>
              ...
              <div className="form-group">
                <label className="checkbox-label">
                  <input
                    type="checkbox"
                    name="estQuizFinal"
                    checked={dataQuiz.estQuizFinal}
                    onChange={detectChangeQuiz}
                  />
                  <span>
                    <i className='bx bx-certification'></i>
                    Quiz final d evaluation (pour certification)
                  </span>
                </label>
                {dataQuiz.estQuizFinal && (
                  <small className="info-quiz-final">
                    <i className='bx bx-info-circle'></i>
                    La meilleure note obtenue à ce quiz conditionnera la délivrance du certificat du cours (75/100 minimum requis).
                  </small>
                )}
              </div>
